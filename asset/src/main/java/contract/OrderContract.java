...
import static model.LicenseKey.*;
...
public class OrderContract implements ContractInterface {
    ...
    public IdResponse GetQuote(Context ctx) {
        ...
        // check asset exists
        if (ctx.getStub().getPrivateData(ADMINMSP_IPDC, assetKey(req.getAssetId())).length == 0) {
            throw new ChaincodeException("asset " + req.getAssetId() + " does not exist");
        }
    ...
    public IdResponse InitiateOrder(Context ctx) {
        ...
        if (order.getStatus().equals(RENEWAL_QUOTE_RECEIVED)) {
            ...
            order.setInitiationDate(ctx.getStub().getTxTimestamp().toString());
    ...
    public IdResponse ApproveOrder(Context ctx) {
        ...
        order.setApprovalDate(ctx.getStub().getTxTimestamp().toString());
    ...
    public LicensesRequest AllocateLicenses(Context ctx) {
        ...
        // if renewal add duration to existing expiration
        String expiration;
        int duration = order.getDuration();
        if (order.getExpiration() != null) {
            expiration = DateFormatter.toInstant(order.getExpiration())
                                 .atZone(ZoneId.from(UTC))
                                 .plusYears(duration)
                                 .toInstant().toString();
        } else {
            expiration = DATE_TIME_FORMATTER.format(ctx.getStub()
                                                       .getTxTimestamp()
                                                       .atZone(ZoneId.from(UTC))
                                                       .plusYears(duration)
                                                       .toInstant());
    ...
    public void SendLicenses(Context ctx) {
        ...
        for (String licenseId : req.getLicenses()) {
            ...
            String key = allocatedLicenseKey(req.getOrderId(), licenseId);
            ctx.getStub().putPrivateData(collection, key, allocated.toByteArray());
        }
    }
    ...
    public void InitiateReturn(Context ctx) {
        ...
        byte[] hash = ctx.getStub().getPrivateDataHash(ADMINMSP_IPDC, key);
        ...
        // check that each license exists in ORG using hash
        for (String licenseId : req.getLicenses()) {
            String licenseKey = allocatedLicenseKey(req.getOrderId(), licenseId);
            hash = ctx.getStub().getPrivateDataHash(accountIPDC(req.getAccount()), licenseKey);
            if (Arrays.equals(hash, EMPTY_HASH_BYTES)) {
                // it's ok to provide a specific error because only users of the org will get here
                throw new ChaincodeException("license " + licenseId + " is not leased by " + req.getAccount());
            }
        }
    ...
    public void DeallocateLicensesFromAccount(Context ctx) throws Exception {
        ...
        for (String license : req.getLicenses()) {
            key = allocatedLicenseKey(req.getOrderId(), license);
    ...
    public void DeallocateLicensesFromSP(Context ctx) {
        ...
        for (String licenseId : licensesRequest.getLicenses()) {
            bytes = ctx.getStub().getPrivateData(ADMINMSP_IPDC, licenseKey(order.getAssetId(), licenseId));
            if (bytes.length == 0) {
            ...
            // write to ADMINMSP
            ctx.getStub().putPrivateData(ADMINMSP_IPDC, licenseKey(order.getAssetId(), licenseId), license.toByteArray());

            // write to ledger to track tx
            ctx.getStub().putState(hashedLicenseKey(order.getAssetId(), licenseId, license.getSalt()), new byte[]{0});
        }
    ...
    @Transaction
    public String[] GetAvailableLicensesForOrder(Context ctx) {
        OrderIdAndAccountRequest req = new OrderIdAndAccountRequest(ctx);

        PDP.canReadLicense(ctx, req.getAccount());

        // get the licenses allocated to the account that do not also have a swid key associated with it
        String collection = accountIPDC(req.getAccount());
        String key = allocatedLicenseKey(req.getOrderId(), "");
        try(QueryResultsIterator<KeyValue> stateByRange = ctx.getStub().getPrivateDataByRange(collection, key, key + "~")) {
            List<String> licenses = new ArrayList<>();
            for (KeyValue next : stateByRange) {
                Allocated allocated = Allocated.fromByteArray(next.getValue());
                String licenseId = allocated.getLicenseId();
                String swidKey = swidKey(req.getOrderId(), licenseId);
                if (ctx.getStub().getPrivateData(collection, swidKey).length != 0) {
                    continue;
                }
            ...
            return licenses.toArray(String[]::new);
        } catch (Exception e) {
            throw new ChaincodeException(e);
        }
    }
    ...
    private void allocateRenewalLicenses(Context ctx, Order order) {
        ...
        for (String licenseId : order.getLicenses()) {
            String lk = licenseKey(order.getAssetId(), licenseId);
            byte[] bytes = ctx.getStub().getPrivateData(ADMINMSP_IPDC, lk);
            if (bytes.length == 0) {
                throw new ChaincodeException("allocated license " + licenseId + " does not exist for asset " + order.getAssetId());
            }
            ...
            ctx.getStub().putPrivateData(ADMINMSP_IPDC, lk, license.toByteArray());
    ...
    private void allocateLicenses(Context ctx, Order order, List<String> licenses) {
        ...
        for (String licenseId : licenses) {
            byte[] bytes = ctx.getStub().getPrivateData(ADMINMSP_IPDC, licenseKey(order.getAssetId(), licenseId));
            if (bytes.length == 0) {
