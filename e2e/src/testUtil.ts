...
export async function submitTx(
    ...
    if (!chaincode) {
        chaincode = getChaincodeName(contractName);
    }

    const contract = await getContract(user, chaincode, chaincode, contractName);
...
function getContractName(func: string): string {
    if (func === 'Bootstrap') {
        ...
        return swid;
    } else if (func === 'GetAllRoles' || func === 'GetAllPrivileges') {
        return ngac;
    }
}
